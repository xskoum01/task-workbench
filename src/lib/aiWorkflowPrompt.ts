...
export function buildAiWorkflowPrompt(task: Task, customer?: Customer): string {
  ...
  lines.push(
    ...
    ...(workKind === 'script' || workKind === 'ribbon' || task.workflowSetup?.devTargetKind === 'script'
      ? ['6. run_implementation_verification runs Dataverse Metadata Check for script/ribbon files automatically (Primarch, read-only) when configured. If it returns needs_configuration, stop and report the exact configuration instructions to the user — do not attempt the check yourself.']
    ...
    '8. For script/ribbon tasks: after writing implementation files call record_ai_implementation_completed (not record_local_test). For plugin tasks: record build/local test results using record_local_test. Record consultant testing, PR review findings, and next steps using the packet guidance.',
  ...
  lines.push(
    ...
    '',
    '1. Re-read the written file and verify every item in workPacket.implementation.businessRules and workPacket.implementation.acceptanceCriteria is satisfied. Fix any violation before continuing.',
    '2. Call `record_ai_implementation_completed` with taskId, filesChanged (array of file paths), and a one-sentence summary. Do not call record_local_test for script/ribbon tasks — this tool replaces it.',
    '3. Call `continue_developer_workflow` with the task ID.',
    '4. Before relying on automated Dataverse Metadata Check or AI Kit review (i.e. before calling run_implementation_verification or record_ai_kit_review_result for the first time in this session, or immediately after any "tool not found"/"bridge is not running" error), call `get_task_workbench_mcp_capabilities` once. If bridgeMode is "offline", or canRunImplementationVerification or canRecordAiKitReview is false, stop and report the exact missingRequiredTools/recommendedAction to the user, asking them to start Task Workbench and reload the MCP server — do not fall back to old manual-modal instructions, and do not fabricate calling record_ai_kit_review_result. Retry from this step only after the user confirms the app is running and the MCP server has been reloaded.',
    '5. If continue_developer_workflow returns nextAction=run_implementation_verification, call `run_implementation_verification` with the task ID. It runs Dataverse Metadata Check automatically (Primarch, read-only) when configured.',
    '6. If it returns nextAction=run_ai_kit_review: read the applicable AI Kit rules (see get_power_platform_ai_kit_status / workPacket.aiKit.rulesFiles) and the target file yourself, review against workPacket.implementation.fieldMappings/validationFields/businessRules/acceptanceCriteria and the Client API rules (no TODO/FIXME/placeholders, no Xrm.Page/autosave, correct Client API usage), then call `record_ai_kit_review_result` with your verdict, then call `run_implementation_verification` again. Label your review as an AI/Claude review, not an independent human review.',
    '7. If it returns status=tooling_error / nextAction=reload_mcp_or_start_app, stop immediately. Report missingRequiredTools to the user and ask them to start Task Workbench and reload the MCP server, then retry run_implementation_verification. Do not claim AI Kit review must be done manually — that is a tooling availability problem, not a workflow requirement.',
    '8. If run_implementation_verification returns fixableFindings, fix the code for each finding, then repeat from step 2 (record_ai_implementation_completed, then continue_developer_workflow, then run_implementation_verification again) until it returns no fixable findings.',
    '9. If it returns nextAction=needs_configuration, stop and report the exact configuration instructions to the user — this cannot be resolved by the AI agent.',
    '10. Stop only when continue_developer_workflow returns wait_for_user (manual web resource upload/form registration/browser Local Test), needs_configuration, tooling_error, or any action requiring explicit user approval.',
    '11. For any requiresUserApproval=true action (branch create, push, Dataverse write), ask the user for explicit confirmation first.',
    '12. If workPacket.implementation.fieldMappings is empty when mappings are expected, stop. Do not write TODO or scaffold code as a substitute for missing mappings.',
    '13. TODO comments in final implementation output are never acceptable. Do not call record_ai_implementation_completed or continue_developer_workflow until every TODO, FIXME, and placeholder comment has been replaced with actual implementation. If the packet provides enough information to replace a TODO, do so. If not, report the blocker.',
  );
