...
export function buildAiWorkflowPrompt(task: Task, customer?: Customer): string {
  ...
  lines.push(
    ...
    '5. If continue_developer_workflow returns nextAction=run_implementation_verification, call it — this also runs Dataverse metadata verification.',
    "6. If run_implementation_verification reports the Dataverse gate as needs_configuration, not run, failed, or warnings not yet accepted (nextAction is needs_configuration or review_dataverse_warnings): stop and report the exact results to the user. Ask whether to fix the code, rerun the check, configure the connection, or accept the warnings — do not proceed past this without the user's answer.",
    '7. If continue_developer_workflow or run_implementation_verification returns nextAction=run_ai_kit_review (run_implementation_verification may also report status=pending_ai_kit_review): review the target file yourself against fieldMappings, validationFields, businessRules, acceptanceCriteria, Claude Project Instructions, and Power Platform AI Kit rules. Then call `record_ai_kit_review_result` with your verdict, always including the full detail payload (reviewedFiles, rulesFiles, checklistFiles, knownPrReviewFiles, checkedItems, plus findings/fixableFindings/nonFixableWarnings/summary) — a bare status with empty detail arrays is recorded as incomplete, not passed. Label it as an AI/Claude review, not an independent human review. Then call `run_implementation_verification` again.',
    '8. If run_implementation_verification or the AI Kit review result returns fixableFindings, fix the code and repeat from step 2.',
    "9. If record_ai_kit_review_result's response reports a gateStatus other than passed (or run_implementation_verification returns nextAction=run_ai_kit_review again), fix the findings or gather the missing detail and call record_ai_kit_review_result again — a status:'passed' call alone is not sufficient.",
    '10. If run_implementation_verification or continue_developer_workflow returns status=tooling_error or nextAction=reload_mcp_or_start_app, stop. Report missingRequiredTools/recommendedAction and ask the user to start Task Workbench and reload the MCP server — this is a tooling problem, not a manual-review requirement.',
    "11. When run_implementation_verification returns nextAction=continue_workflow, this means the Dataverse gate and the AI Kit review gate are resolved — it is NOT a stopping point and NOT a signal to report to the user yet. Local Test is already recorded as not-needed by record_ai_implementation_completed in step 2 — it is not a wait_for_user reason for this AI-managed workflow. nextAction=continue_workflow does NOT mean the artifact has been deployed or tested — Implementation Verification is a pre-deployment code/metadata quality gate only. Do NOT stop here. Immediately proceed to step 11a below in the SAME turn.",
    '11a. Call `continue_developer_workflow` (a different tool from run_implementation_verification). This is required, not optional — the task does not advance out of Development until this call runs. It persists the transition into Deployment & Testing and returns the actual next step.',
  );

  lines.push(
    '',
    'THE LOOP RULE — read this before calling continue_developer_workflow:',
    '* The canonical sequence after Implementation Verification passes is: Deployment & Testing -> Commit & Push -> Pull Request -> Code Review (waiting for colleague review) -> Done.',
    '* Every response from `continue_developer_workflow` has requiresUserApproval and canProceed fields. If requiresUserApproval=false, you must act (call the named recommendedTool, or fix code) and then call `continue_developer_workflow` again — never stop and never report to the user on a requiresUserApproval=false result.',
    '* Stop calling `continue_developer_workflow` only when it returns requiresUserApproval=true, or nextAction is one of: needs_configuration, review_dataverse_warnings, wait_for_manual_deployment, wait_for_deployment_test, wait_for_colleague_code_review. These are the genuine user-action stopping points.',
    '* Loop-safety guard: never call `continue_developer_workflow` twice in a row with no intervening tool call (recommendedTool call, code fix, or record_* call) in between. If it ever returns the exact same nextAction as the immediately preceding call with requiresUserApproval=false and you performed the recommended action in between with no error, stop and report a tooling/state error — do not call it a third time.',
    '',
    "12. Manual deployment (wait_for_manual_deployment): stop and wait. Never deploy anything yourself, and never infer deployment from code creation, metadata verification, AI Kit review, file existence, or commit state — those are automated checks, not evidence of a manual deployment. Only when the user explicitly states in chat that they deployed it (e.g. \"Skript jsem nasadil.\", \"Nasazení je hotové.\", \"Nahrál jsem web resource a publikoval ho.\") — not an ambiguous statement like \"asi je to nasazené\" or \"můžeš pokračovat\" — call `record_manual_deployment` immediately with { taskId, status: 'deployed' }. Do not ask for deployment notes and do not wait for them; notes are optional on a successful confirmation and required only for status='not-needed' (a real user-given reason, never a placeholder). Never claim that you performed the deployment yourself. Manual deployment is never a Pull Request step and is never satisfied by anything commit/push/PR related — do not mention commit, push, or PR while this step is unresolved.",
    '13. Deployment test (wait_for_deployment_test): stop and wait for a real browser/model-driven app test result. Never infer a passed test from automated verification, AI Kit review, or any static/code check. Only when the user explicitly confirms the test passed (e.g. "Otestoval jsem to a funguje to.", "Nasadil jsem to a test prošel.") call `record_deployment_test` immediately with { taskId, status: \'passed\' } — no notes required or requested. If the user reports both the deployment and a passed test in the same message, record deployment first (`record_manual_deployment`), then re-check/continue the workflow state, then record the test (`record_deployment_test`) — never record a test before deployment is recorded. A failed test or a not-needed override may still need a user-given reason. A failed test (fix_code_or_redeploy) means fixing the code or redeploying, then testing again — do not skip ahead. Do not mention commit, push, or PR while this step is unresolved.',
    '14. Commit & Push (prepare_commit / commit_and_push): do not call `commit_task_changes`, `push_task_branch`, or `commit_and_push_task_changes` before the deployment and deployment-test gates both resolve. Recording deployment and/or a passed test is never itself approval to commit or push — Commit & Push remains a separate, later approval gate. Once both gates resolve, call `prepare_commit_for_task` to preview the exact commit, then ask for a SEPARATE explicit user approval of the commit itself before calling `commit_task_changes` — and a further separate approval before `push_task_branch` or `commit_and_push_task_changes`. Do not create a commit, push, or pull request automatically after recording deployment/test confirmations.',
    '15. Pull Request (prepare_pull_request): only after the push is verified, call `prepare_pull_request_for_task` (or the equivalent read-only preview) to show the source/target branch and a draft title/description. Ask for a SEPARATE explicit user approval before creating or recording a pull request — approval to commit/push is never approval to create a PR. If automatic PR creation is unavailable for this repository\'s provider, give the user manual PR-creation instructions and then call `record_pull_request_created` only with a PR URL/number the user (or you, after their approval) actually created — never fabricate one.',
    '16. Stop after the pull request is created or recorded (wait_for_colleague_code_review) and report that the task is now waiting for a colleague\'s code review. Never call this AI/Claude-performed work an independent colleague review — record_ai_kit_review_result is a separate, earlier, automated pre-deployment quality gate, not a substitute for a human reviewing the pull request.',
    '',
    'Any continue_developer_workflow response with requiresUserApproval=true (deployment, deployment test, commit/push, or pull request creation) means: ask the user for explicit confirmation first — never perform or record that step yourself without it. When you stop for a manual action, report ONLY the current manual Deployment & Testing (or later) action that is actually blocking — do not restate steps already resolved.',
    '',
    'If Task Workbench MCP becomes unavailable or a required call fails after 3 retries, stop immediately.',
    '',
