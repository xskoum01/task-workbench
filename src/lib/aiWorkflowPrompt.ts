...
export function buildAiWorkflowPrompt(task: Task, customer?: Customer): string {
  ...
  if (!readiness.isReady) {
    ...
    lines.push(
      ...
      '*   if canApprove=true: call get_developer_work_packet again with the same taskId and use the refreshed packet',
  ...
  lines.push(
    ...
    '',
    `1. If workPacket.canWriteCode is false: (a) if blockingUserAction mentions plan approval, call approve_technical_plan_if_safe with { "taskId": "${task.id}" } — if canApprove=true call get_developer_work_packet again and proceed with the refreshed packet; if canApprove=false stop and report the reasons to the user; (b) for all other blockers stop and report decisionReason and blockingUserAction. Do not create or modify files.`,
