...
export function buildAiWorkflowPrompt(task: Task, customer?: Customer): string {
  ...
  lines.push(
    ...
    '11. Stop when run_implementation_verification returns nextAction=continue_workflow, meaning both the Dataverse gate and the AI Kit review gate are resolved. Otherwise stop only on nextAction=wait_for_user (manual web resource upload/form registration/browser Local Test), needs_configuration, review_dataverse_warnings, tooling_error/reload_mcp_or_start_app, or any requiresUserApproval=true action (ask the user for explicit confirmation first).',
    '',
    'If Task Workbench MCP becomes unavailable or a required call fails after 3 retries, stop immediately.',
    '',
